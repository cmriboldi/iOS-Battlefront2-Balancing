...
extension PriorityQueue where T: Player {
    
    /** Get the index of a player in the heap that has the same battleTeam as the player. Performance: O(n).
     returns nil if there is no player found with that battle team.
     */
    func index(withBattleTeam battleTeam: BattleTeam) -> Int? {
        return heap.nodes.index(where: { $0.battleTeam == battleTeam })
    }
    
    func getNTopNodesAsArray(n: Int) -> [T] {
        var nodes = [T]()
        var heapCopy = heap
        for _ in 0...n {
            if let node = heapCopy.remove() {
                nodes.append(node)
            }
        }
        return nodes
